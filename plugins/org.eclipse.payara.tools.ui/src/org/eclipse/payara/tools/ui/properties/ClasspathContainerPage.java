...
/**
 * This class implements the properties that are shown when Payara is set as a
 * target runtime for a project and the corresponding library container is right
 * clicked and then <code>properties</code> are selected.
 * 
 * <p>
 ...
 * 
 * <p>
 * The properties this dialog implements are the choice between the default
 * libraries for a Payara version that are targeted at application developers,
 * and all the available libraries in Payara.
 * 
 * @author Arjan Tijms
 *
 */
	...
	public void createControl(Composite parent) {
		...
		libraryChoice.addPropertyChangeListener(event -> {
			...
			if (status.isOK()) {
				...
				IPath containerPath = new Path(selection.getPath().segments()[0]).append((String) event.getNewValue());

				selection = JavaCore.newContainerEntry(containerPath, selection.getAccessRules(),
						selection.getExtraAttributes(), selection.isExported());
			} else {
