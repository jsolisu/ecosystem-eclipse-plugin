...
public class AddGenericResourceDataModelProvider extends NewWebClassDataModelProvider {
	...
	/**
	 * Subclasses may extend this method to add their own data model's properties as
	...
	/**
	 * Subclasses may extend this method to provide their own default values for any
	 * of the properties in the data model hierarchy. This method does not accept a
	 * null parameter. It may return null.
	 *
	 ...
	 *
	 * @see
	 * org.eclipse.jst.j2ee.internal.web.operations.NewWebClassDataModelProvider#
	 * isPropertyEnabled(java. lang.String)
	 */
	@Override
	...
	public IStatus validate(String propertyName) {
		if (MIME_TYPE.equals(propertyName)) {
			...
			if (value == null || value.trim().length() == 0) {
				return PayaraToolsPlugin.createErrorStatus(Messages.errorMimeTypeMissing, null);
			}
		}
		...
		if (isPropertyEnabled(propertyName)) {
			if (CONTAINER_PATH.equals(propertyName)) {
				...
				if (value == null || value.trim().length() == 0) {
					return PayaraToolsPlugin.createErrorStatus(Messages.errorContainerPathMissing, null);
				}
			}
			if (CONTAINER_REPRESENTATION_CLASS.equals(propertyName)) {
				return validateRepClass(CONTAINER_REPRESENTATION_CLASS,
						Messages.errorContainerRepresentationClassMissing,
	...
	protected IStatus validateRepClass(String propertyName, String errorMessageKeyMissing,
			String errorMessageKeyInvalid) {
	...
	private IStatus validatePath(String path) {
		if (path == null || path.trim().length() == 0) {
			return PayaraToolsPlugin.createErrorStatus(Messages.errorPathMissing, null);
		}
		if (!isSimplePattern()) {
			...
			while (segments.hasMoreTokens()) {
				...
				if (segment.startsWith("{")) { //$NON-NLS-1$
					if (segment.length() > 2 && segment.endsWith("}")) { //$NON-NLS-1$
						...
						IStatus javaStatus = JavaConventions.validateIdentifier(pathPart, CompilerOptions.VERSION_1_3,
