...
public abstract class AbstractPayaraSelectionHandler extends AbstractHandler {
	...
	protected void showMessageDialog(String msg) {
		new MessageDialog(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), "Cannot Execute this action",
				null, msg, 2, new String[] { "OK" }, 1).open();
	}

	protected void showPageInDefaultBrowser(String url) throws PartInitException, MalformedURLException {
		PlatformUI.getWorkbench().getBrowserSupport().createBrowser(LOCATION_BAR | NAVIGATION_BAR, null, null, null)
				.openURL(new URL(url));
	}
