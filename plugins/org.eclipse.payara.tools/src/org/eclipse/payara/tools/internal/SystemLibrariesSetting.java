	...
	public static SystemLibrariesSetting load(IProject project) {
		try {
			...
			try (InputStream stream = settingsXmlFile.getContents()) {
				settings = (SystemLibrariesSetting) JAXBContext.newInstance(SystemLibrariesSetting.class)
	...
	public File getSourcePath(File jar) {
		for (Library library : libraries) {
			if (jar.equals(new File(library.getPath()))) {
				...
				// Workspace location
				return ResourcesPlugin.getWorkspace().getRoot().getFile(new Path(library.getSource())).getLocation()
						.toFile();
			}
		}
