...
import org.eclipse.payara.tools.sdk.data.DataException;
 ...
 * <p/>
 * <ul>
 * <li><i>Offline:</i> Server is not running</li>
 * <ul>
 * <li>no process</li>
 * <li>no network listeners</li>
 * <li>no administration interface responses</li>
 * </ul>
 * <li><i>Startup/Restart:</i> Server start or restart was requested</li>
 * <ul>
 * <li>active process (PID should change for restart)</li>
 ...
 * <li>valid administration interface responses</li>
 * </ul>
 * <li><i>Shutdown:</i> Server shutdown was requested but server is still
 * running</li>
 * <ul>
 * <li>active process</li>
 * <li>network listeners may or may not be active</li>
 * <li>administration interface may or may not be active</li>
 * </ul>
 * </ul>
 * <p>
 ...
 * </p>
 * <ul>
 * <li><i>Offline:</i> Server is not running
 * <ul>
 * <li>no network listeners
 * <li>no administration interface responses</li>
 * </ul>
...
public enum GlassFishStatus {
	...
	/**
	 * Server start or restart was requested but server is still not fully
	 * responding.
	 */
	STARTUP,
	...
	/**
	 * Server shutdown was requested but server is still running or responding.
	 */
	SHUTDOWN;
	 ...
	 * <p/>
	 * The <code>GlassFishStatus</code> returned represents existing value only if
	 * specified <code>String</code> matches any <code>String</code> returned by
	 ...
	 * @return <code>GlassFishStatus</code> value represented by <code>String</code>
	...
	/**
	 * Initialize GlassFish server status task scheduler to use external executor.
	 * <p/>
	 * This method must be called before adding first GlassFisg server instance into
	 * scheduler.
	 * <p/>
	 *
	 * @param executor External executor to be used in scheduler.
	 */
	public static void initScheduler(final ScheduledThreadPoolExecutor executor) {
		StatusScheduler.init(executor);
	}

	/**
	 * Register GlassFish server instance into scheduler and launch server status
	 * checking jobs.
	 * <p/>
	 ...
	 *         added into scheduler and status checking job was started or
	 ...
	 * @param currentState Notify about current server status after every check when
	 *                     <code>true</code>.
	 * @param newState     Notify about server status change for new states provided
	 *                     as this argument.
	 * @return Value of <code>true</code> when server instance was successfully
	 *         added into scheduler and status checking job was started or
	 *         <code>false</code> otherwise.
	 */
	public static boolean add(final PayaraServer srv, final GlassFishStatusListener listener,
			final boolean currentState, final GlassFishStatus... newState) {
		StatusScheduler scheduler = StatusScheduler.getInstance();
		if (!scheduler.exists(srv)) {
			GlassFishStatusEntity status = new GlassFishStatusEntity(srv);
	 ...
	 * @return GlassFish server instance status. Returns <code>UNKNOWN</code> value
	 *         for unregistered server instance.
	 */
	 ...
	 * <p/>
	 * When status checking is disabled, it will restart it and return current
	 * status which is probably <code>UNKNOWN</code>. If listener is provided, it
	 * will be registered to receive any state change notification following status
	 * checking restart. This listener won't be unregistered automatically so caller
	 * should handle it properly.
	 * <p/>
	 *
	 * @param srv      GlassFish server instance to be searched.
	 * @param listener Server status listener to be registered when status checking
	 *                 is being restarted.
	 * @return GlassFish server instance status. Returns <code>UNKNOWN</code> value
	 *         for unregistered server instance.
	 */
	public static GlassFishStatus getStatus(final PayaraServer srv, final GlassFishStatusListener listener) {
		GlassFishServerStatus status = StatusScheduler.getInstance().get(srv, listener);
		return status != null ? status.getStatus() : GlassFishStatus.UNKNOWN;
	}

	/**
	 * Get current GlassFish server instance {@link GlassFishServerStatus} object.
	 * <p/>
	 * When status checking is disabled, it will restart it and return current
	 * status which is probably <code>UNKNOWN</code>.
	 * <p/>
	 *
	 * @param srv GlassFish server instance to be searched.
	 * @return GlassFish server status {@link GlassFishServerStatus} object. Returns
	 *         <code>null</code> value for unregistered server instance.
	 */
	 ...
	 *         <code>null</code> value for unregistered server instance.
	 */
	public static GlassFishServerStatus get(final PayaraServer srv, final GlassFishStatusListener listener) {
	 ...
	 * <p/>
	 * This will switch status monitoring into startup mode where server is being
	 * checked more often.
	 * <p/>
	 *
	 * @param srv      GlassFish server instance to be switched into startup mode.
	 * @param force    Force startup mode for GlassFish server instance from any
	 *                 state then <code>true</code>.
	 * @param listener Server status listener to be registered together with
	 *                 switching into startup mode.
	 * @param newState Notify about server status change for new states provided as
	 *                 this argument.
	 * @return Value of <code>true</code> when server instance was successfully
	 *         added into scheduler and status checking job was started or
	 *         <code>false</code> otherwise.
	 */
	public static boolean start(final PayaraServer srv, final boolean force, final GlassFishStatusListener listener,
	 ...
	 * <p/>
	 * This will switch status monitoring into shutdown mode where server is being
	 * checked more often.
	 * <p/>
	 *
	...
	/**
	 * Remove GlassFish server instance from scheduler and stop server status
	 * checking jobs.
	 * <p/>
	 * It is possible to call this method for the same GlassFisg server instance
	 ...
	 * @return Value of <code>true</code> when server instance was successfully
	 *         removed from scheduler and status checking job was stopped or
	 *         <code>false</code> when server instance was not registered.
	 */
	 ...
	 * @return Value of <code>true</code> when server instance monitoring was
	 *         suspended or <code>false</code> when server instance is not
	 *         registered.
	 */
	...
	/**
	 * Register server status listener to be notified about current server status
	 * after every check.
	 * <p/>
	 *
	 * @param srv      GlassFish server instance being monitored.
	 * @param listener Server status listener to be registered.
	 * @return Value of <code>true</code> when listener was added or
	 *         <code>false</code> when this listener was already registered or
	 *         GlassFish server instance was not registered.
	 */
	public static boolean addCheckListener(final PayaraServer srv, final GlassFishStatusListener listener) {
	 ...
	 * @param newState Notify about server status change for new states provided as
	 *                 this argument.
	 * @return Value of <code>true</code> when listener was added in at least one
	 *         list or <code>false</code> when this listener was already registered
	 *         in all requested lists or GlassFish server instance was not
	 *         registered.
	 */
	public static boolean addChangeListener(final PayaraServer srv, final GlassFishStatusListener listener,
			final GlassFishStatus... newState) {
		final StatusJob job = StatusScheduler.getInstance().getJob(srv);
		if (job != null) {
	 ...
	 *         registered.
	 */
	public static boolean addErrorListener(final PayaraServer srv, final GlassFishStatusListener listener) {
		final StatusJob job = StatusScheduler.getInstance().getJob(srv);
		if (job != null) {
	 ...
	 *         registered.
	 */
	public static boolean addListener(final PayaraServer srv, final GlassFishStatusListener listener,
			final boolean currentState, final GlassFishStatus... newState) {
		final StatusJob job = StatusScheduler.getInstance().getJob(srv);
		if (job != null) {
	 ...
	 * @return Value of <code>true</code> when listener was found and removed or
	 *         <code>false</code> when listener was not found among registered
	 *         listeners or GlassFish server instance was not registered.
	 */
	public static boolean removeListener(final PayaraServer srv, final GlassFishStatusListener listener) {
		final StatusJob job = StatusScheduler.getInstance().getJob(srv);
		if (job != null) {
	...
	public String toString() {
		...
		default:
			throw new DataException(LOGGER.excMsg(METHOD, "invalidState"));
		}
	}
