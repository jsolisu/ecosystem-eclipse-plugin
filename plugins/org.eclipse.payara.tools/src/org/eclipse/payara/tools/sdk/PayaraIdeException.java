 ...
 * <p>
 * Base exception for GlassFish IDE SDK Exception contains all common code. All
 * exceptions are logging themselves on WARNING level when created.
 * <p>
 *
...
public class PayaraIdeException extends RuntimeException {
	...
	/**
	 * Constructs an instance of <code>GlassFishIdeException</code> without detail
	 * message.
	 */
		...
		if (LOGGER.isLoggable(Level.WARNING)) {
			...
			if (cause != null) {
				...
				sb = new StringBuilder(
						hdr.length() + className.length() + sep.length() + (msg != null ? msg.length() : 0));
				sb.append(hdr);
				sb.append(className);
				sb.append(sep);
				if (msg != null) {
