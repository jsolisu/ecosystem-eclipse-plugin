 ...
 * object. The keys to particular locations are as followed: Installation root -
 * "Base-Root_value" Domain root - "Domain-Root_value"
 * <p/>
 * Minimal <code>__locations</code> command support exists since GlassFish 3.0.1
 * where both Base-Root and Domain-Root values are returned.
 * <p/>
 *
...
public class CommandLocation extends Command {
	 ...
	 *         directory of provided GlassFish server entity and value of
	 *         <code>false</code> that they differs. For remote serve this test
	 *         makes no sense and value of <code>true</code> is always returned.
	 */
	public static boolean verifyResult(final ResultMap<String, String> resultMap, final PayaraServer server) {
		if (!server.isRemote()) {
