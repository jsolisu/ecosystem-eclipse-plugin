...
import static org.eclipse.payara.tools.sdk.utils.ServerUtils.GF_DOMAIN_CONFIG_DIR_NAME;
 ...
 * including the full path, of a file that contains password entries in a
 * specific format.
 * <p/>
 * Note that any password file created to pass as an argument by using the
 * <code>--passwordfile</code> option should be protected with file system
 * permissions. Additionally, any password file being used for a transient
 * purpose, such as setting up SSH among nodes, should be deleted after it has
 * served its purpose.
 * <p/>
 * The entry for a password must have the AS_ADMIN_ prefix followed by the
 * password name in uppercase letters, an equals sign, and the password.
 * <p/>
 ...
 * <li><code>AS_ADMIN_MASTERPASSWORD=master-password</code></li> The entries in
 * this file that are read by subcommands are as follows:
 ...
 * </ul>
 * These password entries are stored in clear text in the password file. To
 * provide additional security, the create-password-alias subcommand can be used
 * to create aliases for passwords that are used by remote subcommands. The
 * password for which the alias is created is stored in an encrypted form. If an
 * alias exists for a password, the alias is specified in the entry for the
 * password as follows:
 * <p/>
...
public class PasswordFile {
	...
	/**
	 * Password file permissions when file is being created before writing it's
	 * content.
	 */
	...
	private String dataToWrite() {
		...
		if (masterPassword != null) {
			len += AS_ADMIN_MASTERPASSWORD.length() + 1 + masterPassword.length();
		}
		if (adminNewPassword != null) {
			len += AS_ADMIN_NEWPASSWORD.length() + 1 + adminNewPassword.length();
		}
	...
	/**
	 * Create password file to be written with access permissions to read and write
	 * by user only.
	 * <p/>
	 *
	 * @return Value of <code>true</code> if new file was created or
	...
	private boolean createFilePosix() {
		...
		try {
			if (Files.notExists(file, new LinkOption[0])) {
				Files.createFile(file, PosixFilePermissions.asFileAttribute(CREATE_FILE_PERMISSIONS));
				success = true;
			} else {
	 ...
	 * <p/>
	 *
	 * @return Value of <code>true</code> when file was successfully written or
	 *         <code>false</code> otherwise.
	 */
	public boolean write() {
		...
		try {
			...
			success = false;
			LOGGER.log(Level.INFO, METHOD, "writeException", new Object[] { file.toString(), ioe.getMessage() });
		} finally {
			if (out != null) {
				try {
					out.close();
					...
					success = false;
					LOGGER.log(Level.INFO, METHOD, "closeException", file.toString());
				}
				finishFilePosix();
			}
		}
