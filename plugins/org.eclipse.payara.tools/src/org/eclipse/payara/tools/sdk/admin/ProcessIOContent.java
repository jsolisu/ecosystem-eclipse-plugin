...
/**
 * Content to verify on server administration command execution standard output
 * and data to send on standard input.
 * <p/>
 *
 * @author Tomas Kraus
 */
public class ProcessIOContent {
	...
	protected abstract static class Token {
		...
		/**
		 * Array of match indicators for individual success input strings. This value
		 * shall not be null.
		 */
		private final boolean matchSuccess[];

		/**
		 * Array of match indicators for individual error input strings. This value
		 * shall not be null.
		 */
		...
		/**
		 * Create an instance of abstract token expecting set of strings on standard
		 * output.
		 * <p/>
		 *
		 * @param inputSuccess Array of input strings considered as successful response
		 *                     to be matched before prompt.
		 * @param inputError   Array of input strings considered as error response to be
		 *                     matched before prompt.
		 */
		protected Token(final String[] inputSuccess, final String[] inputError) {
			...
			for (short i = 0; i < lenSuccess; i++) {
				...
				outputStrings.add(inputSuccess[i], new TreeNode(i, ProcessIOResult.SUCCESS));
			...
			for (short i = 0; i < lenError; i++) {
				matchError[i] = false;
				outputStrings.add(inputError[i], new TreeNode(i, ProcessIOResult.ERROR));
		 ...
		 * <p/>
		 *
		 * @return Value of <code>true</code> when all successful input strings were
		 *         matched or <coe>false</code> otherwise.
		 */
	...
	protected static class OutputToken extends Token {
		...
		/**
		 * Create an instance of output token expecting set of strings on standard
		 * output.
		 * <p/>
		 *
		 * @param inputSuccess Array of input strings considered as successful response
	...
	protected static class InputToken extends Token {
		...
		/**
		 * Create an instance of input token expecting custom input prompt and set of
		 * strings on standard output.
		 * <p/>
		 *
		 ...
		 * @param output Output to be sent after all input strings are matched and
		 *               prompt is received.
		 */
		protected InputToken(final String prompt, final String[] input, final String output) {
			super(input, null);
			this.prompt = prompt;
		}
		 ...
		 * <p/>
		 *
		 * @return Process input prompt different from global input prompt or
		 *         <code>null</code> when no such prompt is set.
		 */
		@Override
		protected String getPrompt() {
			return prompt;
	...
	/**
	 * Process input prompt. Data on standard input are expected after sending this
	 ...
	 * Creates an instance of server administration command execution content.
	 * <p/>
	 * No input prompt is set. This constructor may be used for commands which are
	 * expecting no input.
	 */
	 ...
	 * <p/>
	 * All strings in <code>inputSuccess</code> array must be matched for successful
	 ...
	 *                     matched before prompt.
	 */
	public void addOutput(final String[] inputSuccess, final String[] inputError) {
	 ...
	 *               prompt is received.
	 */
	public void addInput(final String prompt, final String[] input, final String output) {
	 ...
	 * Add next expected process input dependent on matched process output.
	 * <p/>
	 * <code>inputSuccess</code> string must be matched for successful evaluation.
	 * Any matching string from <code>inputError</code> will cause evaluation as
	 * error.
	 * <p/>
	 *
	 * @param prompt Process input prompt different from global input prompt.
	 ...
	 * <p/>
	 * Next token from list or <code>null</code> when there are no more tokens in
	 * the list.
	 */
