...
import org.eclipse.payara.tools.sdk.utils.CyclicStringBuffer;
...
public class ProcessIOParser {
	 ...
	 * <p/>
	 * Process output is being read as text lines separated by prompt string, CR or
	 * CRLF. Each finished line is searched for provided process IO content.
	 */
	protected static class Parser {
		...
		protected enum Input {
			 ...
			 */
			protected static Input value(final char c, final String prompt, final CyclicStringBuffer promptBuff) {
		 ...
		 * <p/>
		 *
		 * @param content Content to verify on server administration command execution
		 *                IO.
		 */
	 ...
	 * @param ioContent Content to verify on server administration command execution
	 *                  IO.
	 */
	public ProcessIOParser(final Writer stdIn, final Reader stdOut, final ProcessIOContent ioContent) {
		this.stdIn = stdIn;
		this.stdOut = stdOut;
	...
	/**
	 * Verify process output streams against content verification data provided in
	 * constructor as <code>ioContent</code> argument.
	 * <p/>
	 *
	 * @return Process output streams verification result.
	 * @throws IOException When there is an issue with reading process output
	 *                     streams.
	 */
