...
import org.eclipse.payara.tools.sdk.GlassFishToolsConfig;
...
public abstract class Runner implements Callable<Result> {
	...
	protected static class StateChange {
		 ...
		 */
		protected StateChange(final Runner runner, final TaskState taskState, final TaskEvent taskEvent,
				final String... args) {
			this.runner = runner;
		...
		/**
		 * Update task state value in <code>Result</code> object and notify all
		 * registered command execution state listeners about command execution state
		 * change.
		 * <p/>
		 *
	...
	/**
	 * Allows to initialize this class to use external <code>Authenticator</code>.
	 * <p/>
	 * This method must be called before first usage of <code>call()</code> method.
	 * <p/>
	 *
	 * @param authenticator External authenticator for GlassFish servers to be
	 *                      supplied.
	 */
	 ...
	 *
	 * @return External <code>Authenticator</code> for GlassFish servers or
	 *         <code>null</code> if no external <code>Authenticator</code> was set.
	 */
	...
	/**
	 * Convert <code>String</code> constant representing <code>boolean</code> value
	 * to it's original <code>boolean</code> value.
	 * <p/>
	 *
	 ...
	 * responsible for correct <code>TaskState</code> and receiveResult value
	 * handling.
	 */
	Result<ValueProcess> result;
	...
	/**
	 * Create <code>Result</code> object corresponding to command execution value to
	 * be returned.
	 */
	protected abstract Result<ValueProcess> createResult();

	/**
	 * Send information to the server via HTTP POST.
	 * <p/>
	 *
	 * @return <code>true</code> if using HTTP POST to send to server or
	 ...
	 * <p/>
	 *
	 * @return <code>true</code> when gzip format is accepted, <code>false</code>
	 *         otherwise.
	 */
	...
	/**
	 * Override to change the type of HTTP method used for this command. Default is
	 * GET.
	 * <p/>
	 *
	 ...
	 * <p/>
	 * In some cases command has to send data to server. To do that, override this
	 * method.
	 * <p/>
	 *
	 * @param hconn
	 * @throws IOException
	 */
	protected abstract void handleSend(final HttpURLConnection hconn) throws IOException;
	 ...
	 */
	protected abstract boolean readResponse(final InputStream in, final HttpURLConnection hconn);

	/**
	 * Override to parse, validate, and/or format any data read from the server in
	 * readResponse() / readManifest().
	 * <p/>
	 *
	...
	/**
	 * Override to set the content-type of information sent to the server. Default
	 * is null (not set).
	 * <p/>
	 *
	 ...
	 * <p/>
	 *
	 * @return <code>Command</code> string with <code>query</code> parameters
	 *         appended.
	 */
	String getCommandWithQuery() {
		...
		if (query == null) {
			...
			StringBuilder sb = new StringBuilder(commandString.length() + 1 + query.length());
	 ...
	 * @param display   Display this event in GUI (Transformed to
	 *                  <code>String</code> value containing <code>"true"</code>
	 *                  value when user should be notified or <code>"true"</code> or
	...
	String[] stateChangeArgs(final String exMessage, final boolean display) {
		return new String[] { server.getName(), command.getCommand(), exMessage, Boolean.toString(display) };
	}

	/**
	 * Build arguments for operationStateChanged method listening for state changes.
	 * <p/>
	 * Send <code>false</code> display argument to operationStateChanged method to
	 * not display GUI message about this event.
	 * <p/>
	 *
	 * @param exMessage Exception message.
	 * @return Arguments for operationStateChanged method listening for state
	...
	String[] stateChangeArgs(final String exMessage) {
		return new String[] { server.getName(), command.getCommand(), exMessage, Boolean.toString(false) };
	}
	 ...
	 * change.
	 * <p/>
	 * This method is not responsible for setting receiveResult value returned by
	 * GlassFish. This method is used after task is submitted into
	...
	/**
	 * Used with external execution code to prepare initial task state before task
	 ...
	 * <p/>
	 *
	 * @return <code>Future</code> object to retrieve receiveResult of asynchronous
	 *         execution.
	 */
	...
	/**
	 * Prepare headers for HTTP connection. This handles all common headers for all
	 * implemented command interfaces (REST, HTTP, ...).
	 * <p/>
	 *
	 * @param conn Target HTTP connection.
	 * @throws <code>CommandException</code> if there is a problem with setting the
	 * headers.
	 */
	protected void prepareHttpConnection(final HttpURLConnection conn) throws CommandException {
		...
		LOGGER.log(Level.FINEST, METHOD, "setting",
				new Object[] { new Integer(HTTP_CONNECTION_TIMEOUT), adminUser, adminPassword });
		try {
			conn.setRequestMethod(getRequestMethod());
		} catch (ProtocolException pe) {
			throw new CommandException(CommandException.RUNNER_HTTP_HEADERS, pe);
		}
	...
	protected boolean handleReceive(final HttpURLConnection hconn) throws IOException {
	 ...
	 * <p/>
	 * Depending on GlassFisg Tooling Library configuration proxy usage for loopback
	 * addresses may be suppressed.
	 * <p/>
	 *
	 ...
	 *                     connection.
	 */
	private static URLConnection openURLConnection(final URL urlToConnectTo) throws IOException {
	...
	private StateChange handleHTTPConnection() throws IOException {
		...
		do { // deal with possible redirects from 3.1
			...
			LOGGER.log(Level.FINEST, METHOD, "connect",
					new Object[] { server.getHost(), Integer.toString(server.getAdminPort()) });
	 ...
	 * @return State change request when <code>call()</code> method should exit.
	 */
	private StateChange handleHTTPResponse(final int responseCode) throws IOException {
		...
		if (responseCode == HttpURLConnection.HTTP_UNAUTHORIZED || responseCode == HttpURLConnection.HTTP_FORBIDDEN) {
			...
			if (null == newUrl || "".equals(newUrl.trim())) {
				LOGGER.log(Level.SEVERE, METHOD, "invalidRedirect", urlToConnectTo.toString());
			} else {
	...
	private void logCommandFailure(final String method) {
		LOGGER.log(Level.FINE, method, "failure",
				new Object[] { hconn.toString(), hconn.getContentType(), hconn.getContentEncoding() });
	...
	public Result call() {
		...
		int retries = 1;
		LOGGER.log(Level.FINEST, METHOD, "thread", new Object[] { commandUrl, Thread.currentThread().getName() });
		try {
			...
			while (!httpSucceeded && retries-- > 0) {
				try {
					...
					if (conn instanceof HttpURLConnection) {
						...
						if (!auth) {
							return handleStateChange(TaskState.FAILED, TaskEvent.AUTH_FAILED,
									stateChangeArgs(null, true));
						} else if (!commandSucceeded && !isSilentFailureAllowed() && LOGGER.isLoggable(Level.FINE)) {
							logCommandFailure(METHOD);
						}
						httpSucceeded = true;
					} else {
						LOGGER.log(Level.INFO, METHOD, "unexpectedConnection", urlToConnectTo);
					}
					...
					return handleStateChange(TaskState.FAILED, TaskEvent.EXCEPTION,
							stateChangeArgs(ce.getLocalizedMessage()));
				} catch (IOException ex) {
					if (retries <= 0) {
						return handleStateChange(TaskState.FAILED, TaskEvent.EXCEPTION,
								stateChangeArgs(ex.getLocalizedMessage()));
					}
				} finally {
					if (null != hconn) {
						hconn.disconnect();
		...
		if (commandSucceeded) {
			return handleStateChange(TaskState.COMPLETED, TaskEvent.CMD_COMPLETED, stateChangeArgs(null));
		} else {
			return handleStateChange(TaskState.FAILED, TaskEvent.CMD_FAILED, stateChangeArgs(null));
		}
	}
