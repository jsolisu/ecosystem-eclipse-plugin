...
public class RunnerAsadminChangeAdminPassword extends RunnerAsadmin {
	...
	/**
	 * Specifies the parent directory of the domain specified in the --domain_name
	 * option.
	 */
	...
	private static String query(final PayaraServer server, final Command command) {
		...
		StringBuilder sb = new StringBuilder(DOMAIN_NAME_PARAM.length() + 1 + domainName.length() + 1
	 ...
	 */
	public RunnerAsadminChangeAdminPassword(final PayaraServer server, final Command command) {
		...
		if (command instanceof CommandChangeAdminPassword) {
			...
			throw new CommandException(LOGGER.excMsg(METHOD, "illegalInstance"));
		}
	...
	protected ProcessIOContent createProcessIOContent() {
		...
		processIOContent.addOutput(new String[] { "Command", "executed successfully" },
