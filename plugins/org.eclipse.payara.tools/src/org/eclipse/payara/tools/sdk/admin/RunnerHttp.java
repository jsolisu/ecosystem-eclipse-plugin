...
public class RunnerHttp extends Runner {
	 ...
	 */
	static int queryPropertiesLength(final Map<String, String> properties, final String paramName) {
	 ...
	 */
	static int queryLibrariesLength(final File[] libraries, final String paramName) {
		int propertyLength = 0;
	 ...
	 */
	static StringBuilder queryPropertiesAppend(StringBuilder sb, final Map<String, String> properties,
			final String paramName, final boolean separator) {
		if (null != properties && properties.size() > 0) {
			Set<Map.Entry<String, String>> entrySet = properties.entrySet();
			boolean first = true;
			if (separator) {
	 ...
	 */
	static StringBuilder queryLibrariesAppend(StringBuilder sb, final File[] libraries, final String paramName,
	...
	/**
	 * Check <code>Manifest</code> <i>exit-code<i> attribute for result. Missing
	 * value or any value differed to< code>"FAILED"</code> is considered as
	 * success.
	 * <p/>
	 ...
	 * <code>Manifest</code> instance life cycle is started in
	 * <code>readResponse</code> method where manifest returned from server is read
	...
	/**
	 * Constructs an instance of administration command executor using HTTP
	 * interface. This constructor allows child classes to set their own path.
	 * <p/>
	 ...
	 */
	RunnerHttp(final PayaraServer server, final Command command, final String query) {
	 ...
	 * @param query   Query string for this command.
	 */
	protected RunnerHttp(final PayaraServer server, final Command command, final String path, final String query) {
	...
	protected String constructCommandUrl() throws CommandException {
		...
		if (null == url) {
			protocol = getHttpListenerProtocol(host, port, ":::" + command.getCommand() + "?" + query);
		} else if (!(url.contains("ee6wc"))) {
			protocol = getHttpListenerProtocol(host, port, url + ":::" + command.getCommand() + "?" + query);
		} else if (url.contains("ee6wc") && (null == domainsDir || "".equals(domainsDir))) {
			protocol = "https";
		}
		URI uri;
		try {
			uri = new URI(protocol, null, host, port, path + command.getCommand(), query, null);
	...
	/**
	 * Reads response from server and stores it into internal <code>Manifest</code>
	 * object. Value of <i>exit-code<i> attribute is verified to detect if command
	 * completed successfully. If not, <i>message</i> value is checked for "please
	 * wait" <code>String</code> to eventually set <code>retry</code> value to
	 * <code>true</code>.
	 * <p/>
	...
	protected boolean readResponse(InputStream in, HttpURLConnection hconn) {
		...
		try {
			...
			throw new CommandException(CommandException.HTTP_RESP_IO_EXCEPTION, ioe);
		}
	...
	protected boolean processResponse() {
		...
		result.auth = auth;
		Logger.log(Level.FINEST, "Manifest message: {0}", new Object[] { result.value });
	...
	/**
	 * Override to provide the lastModified date for data source whose inputstream
	 * is returned by getInputStream. Must not return null if getInputStream() does
	 * not return null;
	 *
	 ...
	 * <p/>
	 * Response processing helper method. Manifest must contain valid data.
	 * <code>Manifest</code> instance is always created in <code>readResponse</code>
	 * method so no <code>null</code> check is required here.
	 * <p/>
	 *
	 * @param manifest Manifest from server response.
	 * @return Value of <i>message<i> attribute or <code>null</code> if no such
	 *         attribute exists.
	 */
