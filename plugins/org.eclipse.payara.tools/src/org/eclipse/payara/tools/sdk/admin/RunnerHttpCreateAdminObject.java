 ...
 * <p/>
 * Contains code for create administered object command. Class implements
...
public class RunnerHttpCreateAdminObject extends RunnerHttp {
	...
	private static String query(final Command command) {
		...
		StringBuilder sb = new StringBuilder(JNDI_NAME_PARAM.length() + 1 + jndiName.length() + ENABLED_PARAM.length()
				+ 1 + toString(enabled).length() + (isResType ? RESOURCE_TYPE_PARAM.length() + 1 + resType.length() : 0)
				+ (isRaname ? RA_NAME_PARAM.length() + 1 + raName.length() : 0)
				+ queryPropertiesLength(((CommandCreateAdminObject) command).properties, PROPERTY_PARAM));
		// Build query string
	 ...
	 * @param command GlassFish server administration command entity.
	 */
	public RunnerHttpCreateAdminObject(final PayaraServer server, final Command command) {
