 ...
 * <p/>
 * Contains code for create connector resource command. Class implements
...
public class RunnerHttpCreateConnector extends RunnerHttp {
	...
	private static String query(final Command command) {
		...
		StringBuilder sb = new StringBuilder(JNDI_NAME_PARAM.length() + 1 + jndiName.length() + ENABLED_PARAM.length()
				+ 1 + toString(enabled).length() + (isPoolName ? POOL_NAME_PARAM.length() + 1 + poolName.length() : 0)
				+ queryPropertiesLength(((CommandCreateConnector) command).properties, PROPERTY_PARAM));
		// Build query string
	 ...
	 * @param command GlassFish server administration command entity.
	 */
	public RunnerHttpCreateConnector(final PayaraServer server, final Command command) {
