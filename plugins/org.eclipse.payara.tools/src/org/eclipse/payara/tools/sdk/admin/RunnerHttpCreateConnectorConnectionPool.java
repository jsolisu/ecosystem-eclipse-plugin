 ...
 * <p/>
 * Contains code for create EIS connection pool command. Class implements
...
public class RunnerHttpCreateConnectorConnectionPool extends RunnerHttp {
	...
	private static String query(final Command command) {
		...
		boolean isConnectionDefinition = connectionDefinition != null && connectionDefinition.length() > 0;
		// Calculate StringBuilder initial length to avoid resizing
		StringBuilder sb = new StringBuilder(POOL_NAME_PARAM.length() + 1 + poolName.length()
				+ (isRaname ? RA_NAME_PARAM.length() + 1 + raName.length() : 0)
				+ (isConnectionDefinition ? CONNECTION_DEFINITION_PARAM.length() + 1 + connectionDefinition.length()
						: 0)
				+ queryPropertiesLength(((CommandCreateConnectorConnectionPool) command).properties, PROPERTY_PARAM));
		if (isRaname) {
	 ...
	 * @param command GlassFish server administration command entity.
	 */
	public RunnerHttpCreateConnectorConnectionPool(final PayaraServer server, final Command command) {
