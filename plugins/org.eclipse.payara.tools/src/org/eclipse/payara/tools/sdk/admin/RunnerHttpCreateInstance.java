...
public class RunnerHttpCreateInstance extends RunnerHttp {
	...
	private static String query(Command command) {
		...
		StringBuilder sb = new StringBuilder(DEFAULT_PARAM.length() + 1 + name.length() + 1 + NODE_PARAM.length() + 1
				+ node.length() + (cluster != null ? 1 + CLUSTER_PARAM.length() + 1 + cluster.length() : 0));
	 ...
	 */
	public RunnerHttpCreateInstance(final PayaraServer server, final Command command) {
