 ...
 * <p/>
 * Contains code for create JDBC connection pool command. Class implements
 * GlassFish server administration functionality trough HTTP interface.
 * <p/>
 *
...
public class RunnerHttpCreateJDBCConnectionPool extends RunnerHttp {
	...
	/**
	 * Create JDBC connection pool command <code>connectionpoolid</code> parameter
	 * name.
	 */
	 ...
	 *                <code>CommandCreateJDBCConnectionPool</code> instance is
	 *                expected.
	...
	private static String query(final Command command) {
		...
		boolean isDataSourceClassName = dataSourceClassName != null && dataSourceClassName.length() > 0;
		boolean isResType = resType != null && resType.length() > 0;
		// Calculate StringBuilder initial length to avoid resizing
		StringBuilder sb = new StringBuilder(CONN_POOL_ID_PARAM.length() + 1 + connectionPoolId.length()
				+ (isDataSourceClassName ? DS_CLASS_NAME_PARAM.length() + 1 + dataSourceClassName.length() : 0)
				+ (isResType ? RESOURCE_TYPE_PARAM.length() + 1 + resType.length() : 0)
				+ queryPropertiesLength(((CommandCreateJDBCConnectionPool) command).properties, PROPERTY_PARAM));
		// Build query string
	 ...
	 * @param command GlassFish server administration command entity.
	 */
	public RunnerHttpCreateJDBCConnectionPool(final PayaraServer server, final Command command) {
