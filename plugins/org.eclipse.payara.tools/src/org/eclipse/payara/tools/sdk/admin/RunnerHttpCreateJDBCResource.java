...
/**
 * GlassFish server create JDBC resource administration command execution using
 * HTTP interface.
 * <p/>
 * Contains code for create JDBC resource command. Class implements GlassFish
 * server administration functionality trough HTTP interface.
 * <p/>
 *
...
public class RunnerHttpCreateJDBCResource extends RunnerHttp {
	...
	private static String query(final Command command) {
		...
		StringBuilder sb = new StringBuilder(CONN_POOL_ID_PARAM.length() + 1 + connectionPoolId.length()
				+ (isJndiName ? JNDI_NAME_PARAM.length() + 1 + jndiName.length() : 0)
				+ (isTarget ? TARGET_PARAM.length() + 1 + target.length() : 0)
				+ queryPropertiesLength(((CommandCreateJDBCResource) command).properties, PROPERTY_PARAM));
		// Build query string
		...
		queryPropertiesAppend(sb, ((CommandCreateJDBCResource) command).properties, PROPERTY_PARAM, true);
		return sb.toString();
	}
	 ...
	 * @param command GlassFish server administration command entity.
	 */
	public RunnerHttpCreateJDBCResource(final PayaraServer server, final Command command) {
