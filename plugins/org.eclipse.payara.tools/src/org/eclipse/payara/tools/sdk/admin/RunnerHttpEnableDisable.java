...
/**
 * GlassFish Server <code>enable</code> and <code>disable</code> administration
 * command execution using HTTP interface.
 * <p/>
 * Contains common code for enable and disable commands. Individual child
 * classes are not needed at this stage. Class implements GlassFish server
...
public class RunnerHttpEnableDisable extends RunnerHttp {
	...
	private static String query(final Command command) {
		...
		StringBuilder sb = new StringBuilder(DEFAULT_PARAM.length() + 1 + name.length()
	 ...
	 * @param command GlassFish Server Admin Command Entity.
	 */
	public RunnerHttpEnableDisable(final PayaraServer server, final Command command) {
