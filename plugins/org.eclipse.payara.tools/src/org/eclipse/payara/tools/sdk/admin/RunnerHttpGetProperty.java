...
public class RunnerHttpGetProperty extends RunnerHttp {
	 ...
	 */
	public RunnerHttpGetProperty(final PayaraServer server, final Command command) {
	...
	/**
	 * Create <code>ResultMap</code> object corresponding to server get property
	...
	protected boolean processResponse() {
		...
		for (String encodedkey : manifest.getEntries().keySet()) {
			...
			try {
				...
				LOGGER.log(Level.INFO, METHOD, "unsupportedEncoding", encodedkey);
			...
			if (equalsIndex >= 0) {
				...
				try {
					...
					result.value.put(keyPart, URLDecoder.decode(valuePart, "UTF-8")); // single decode
					result.value.put(keyPart, URLDecoder.decode(result.value.get(keyPart), "UTF-8"));
				} catch (UnsupportedEncodingException ex) {
					LOGGER.log(Level.INFO, METHOD, "unsupportedEncoding", result.value.get(keyPart));
				} catch (IllegalArgumentException iae) {
