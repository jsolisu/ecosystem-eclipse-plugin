...
public class RunnerHttpListResources extends RunnerHttpTarget {
	 ...
	 */
	public RunnerHttpListResources(final PayaraServer server, final Command command) {
	...
	protected boolean processResponse() {
		String resourcesAttr = manifest.getMainAttributes().getValue("children");
		String[] resources = resourcesAttr != null ? resourcesAttr.split(ServerUtils.MANIFEST_RESOURCES_SEPARATOR)
				: null;
		...
		if (resources != null) {
			for (String resource : resources) {
				...
				try {
					...
					throw new CommandException(CommandException.HTTP_RESP_UNS_ENC_EXCEPTION, uee);
				}
