...
/**
 * GlassFish Server <code>redeploy</code> Admin Command Execution using HTTP
 * interface.
 * <p/>
...
public class RunnerHttpRedeploy extends RunnerHttp {
	...
	private static String query(final Command command) {
		...
		boolean first = true;
		StringBuilder sb = new StringBuilder(queryPropertiesLength(((CommandRedeploy) command).properties,
				PROPERTIES_PARAM) + queryLibrariesLength(((CommandRedeploy) command).libraries, LIBRARIES_PARAM)
				...
				+ (((CommandRedeploy) command).keepState ? KEEP_STATE_PARAM.length() + 1 + keepState.length() : 0));
	 ...
	 */
	public RunnerHttpRedeploy(final PayaraServer server, final Command command) {
