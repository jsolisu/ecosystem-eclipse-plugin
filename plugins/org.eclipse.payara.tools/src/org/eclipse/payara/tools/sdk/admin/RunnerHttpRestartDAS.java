...
/**
 * GlassFish restart DAS administration command with <code></code> query
...
public class RunnerHttpRestartDAS extends RunnerHttp {
	...
	static String query(final Command command) {
		if (command instanceof CommandRestartDAS) {
			...
			int boolValSize = FALSE_VALUE.length() > TRUE_VALUE.length() ? FALSE_VALUE.length() : TRUE_VALUE.length();
			StringBuilder sb = new StringBuilder(DEBUG_PARAM.length() + boolValSize + 1);
	 ...
	 * @param command GlassFish server administration command entity.
	 */
	public RunnerHttpRestartDAS(final PayaraServer server, final Command command) {
