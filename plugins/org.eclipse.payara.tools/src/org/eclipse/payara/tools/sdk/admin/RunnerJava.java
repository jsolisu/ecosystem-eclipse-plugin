...
abstract class RunnerJava extends Runner {
	 ...
	 * <p/>
	 * {@link CommandException} is thrown when configuration object value is
	 * <code>null</code>.
	 * <p/>
	 *
	 * @param server GlassFish server entity object.
	 * @return GlassFish server features configuration which is not
	 *         <code>null</code>.
	 */
	static GlassFishConfig getServerConfig(final PayaraServer server) {
		final String METHOD = "getServerConfig";
		GlassFishConfig config = GlassFishConfigManager
				.getConfig(ConfigBuilderProvider.getBuilderConfig(server.getVersion()));
		if (config == null) {
			throw new CommandException(LOGGER.excMsg(METHOD, "noConfig"), server.getVersion());
		}
		return config;
	}

	/**
	 * Get {@link JavaSESet} instance for provided GlassFish server features
	 * configuration.
	 * <p/>
	 *
	 ...
	 */
	private static String getJavaVM(final PayaraServer server, final CommandJava command) {
	 ...
	 */
	public RunnerJava(final PayaraServer server, final Command command, final String query) {
	 ...
	 * <p/>
	 *
	 * @return Value of <code>true</code> when Java VM executable version is known
	 *         and supported by provided GlassFish server or <code>false</code>
	 *         otherwise.
	 */
	boolean verifyJavaVM() {
		...
		if (javaVersion == null) {
			...
			LOGGER.log(Level.FINEST, METHOD, "info", new Object[] { javaVMExe, javaVersion.toString() });
		}
		...
		if (!platforms.contains(javaVersion.toPlatform())) {
			LOGGER.log(Level.INFO, METHOD, "unsupported", new Object[] { javaVMExe, server.getName() });
			return false;
		}
		return true;
	}
	...
	void setProcessCurrentDir(ProcessBuilder pb) {
		...
		if (domainsFolder != null && domainsFolder.length() > 0) {
			File currentDir = new File(ServerUtils.getDomainConfigPath(domainsFolder));
			if (currentDir.exists()) {
				LOGGER.log(Level.FINEST, METHOD, "dir", new Object[] { server.getName(), currentDir });
