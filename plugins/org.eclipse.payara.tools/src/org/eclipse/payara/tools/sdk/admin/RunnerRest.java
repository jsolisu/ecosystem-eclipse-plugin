...
import org.eclipse.payara.tools.sdk.admin.response.ResponseParserFactory;
...
public class RunnerRest extends Runner {
	 ...
	 */
	public RunnerRest(final PayaraServer server, final Command command, final String query) {
	 ...
	 * @param query   Query string for this command.
	 */
	public RunnerRest(final PayaraServer server, final Command command, final String path, final String query) {
	 ...
	 * @param separator
	 */
	protected void appendProperties(StringBuilder sb, final Map<String, String> properties, final String paramName,
	...
	protected String constructCommandUrl() throws CommandException {
		...
		URI uri;
		try {
			uri = new URI(protocol, null, server.getHost(), server.getAdminPort(), path + command.getCommand(), query,
					null);
	...
	/**
	 * Method returns content type in which the server is asked to return the
	 * response.
	 * <p/>
	 *
