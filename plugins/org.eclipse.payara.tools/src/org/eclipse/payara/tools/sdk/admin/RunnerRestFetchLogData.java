...
/**
 * GlassFish Server <code>view-log</code> Administration Command Execution using
 * HTTP interface.
 * <p/>
...
public class RunnerRestFetchLogData extends RunnerRest {
	...
	private static String query(Command command) {
		if (command instanceof CommandFetchLogData) {
			...
			throw new CommandException(CommandException.ILLEGAL_COMAND_INSTANCE);
		}
	 ...
	 * holder. Instance life cycle is started in <code>readResponse</code> method
	 * where log returned from server is read and stored internally.
	 */
	 ...
	 * <p/>
	 * This header contains the entire URL to pass to the GET method to return the
	 * changes since the last call. You can use this header in client applications
	 * to get all log entries that were added in particular interval.
	 */
	 ...
	 */
	public RunnerRestFetchLogData(final PayaraServer server, final Command command) {
	...
	/**
	 * Create <code>ResultLog</code> object corresponding to server log command
	 ...
	 * <code>List</code>. Content of HTTP header <code>X-Text-Append-Next</code> is
	 * stored in internal <code>headerAppendNext</code> variable.
	 * <p/>
	 * It's not necessary close the stream parameter when finished. Caller will take
	 * care of that. But this method uses additional stream handlers for
	 * <code>gzip</code> compression and buffered reading so it should close them.
	 * <p/>
	 *
	 * @param in Stream to read data from.
	 * @return <code>true</code> if response <code>X-Text-Append-Next</code> HTTP
	 *         header contains some parameters (e.g. ?start=&lt;number&gt;) or
