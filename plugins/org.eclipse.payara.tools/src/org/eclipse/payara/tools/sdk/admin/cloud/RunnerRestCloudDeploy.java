...
public class RunnerRestCloudDeploy extends RunnerRestCloud {
	 ...
	 * interface for deploy command.
	 * <p/>
	 *
	 * @param server  GlassFish server entity object.
	 * @param command GlassFish server administration command entity.
	 */
	public RunnerRestCloudDeploy(final PayaraServer server, final Command command) {
	...
	protected void handleSend(HttpURLConnection hconn) throws IOException {
		...
		if (command.path == null) {
			throw new PayaraIdeException("The path attribute of deploy command" + " has to be non-empty!");
		}
	...
	private void writeBinaryFile(OutputStreamWriter writer, OutputStream output, File file) throws IOException {
		writer.append("--" + multipartBoundary).append(NEWLINE);
		writer.append("Content-Disposition: form-data; name=\"warFile\"; filename=\"" + file.getAbsolutePath() + "\"")
				.append(NEWLINE);
