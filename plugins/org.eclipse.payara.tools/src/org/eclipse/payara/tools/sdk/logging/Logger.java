...
package org.eclipse.payara.tools.sdk.logging;
...
public class Logger {
	...
	/**
	 * Find or create a logger for GlassFish IDE SDK. If a logger has already been
	 ...
	 * to its parent's Handlers. It will be registered in the LogManager global name
	 * space.
	 * <p>
	 *
	 ...
	 * @param locales Add locale suffix when <code>true</code> or do not add it when
	 *                <code>false</code>.
	 * @return Messages catalog file name with properties file suffix.
	 */
	private static String buildPropertiesFileName(final String file, final boolean locales) {
	 ...
	 * <p/>
	 * Properties catalog for package of provided class will be loaded from provided
	 * file when missing.
	 * <p/>
	 *
	 * @param file     Messages catalog file name.
	 ...
	 * @return Message from package properties catalog map with given key.
	 */
	private static String message(final String file, final Map<Package, Properties> propsMap, final Class c,
			final String key) {
		...
		synchronized (propsMap) {
			...
			if (props == null) {
				...
				URL url = JavaUtils.getPropertiesURL(c, buildPropertiesFileName(file, false));
				if (url != null) {
					try {
						url.openStream();
	...
	public String buildKey(String method, String key) {
		...
		return new StringBuilder(className.length() + method.length() + key.length() + 2).append(className)
				.append(KEY_SEPARATOR).append(method).append(KEY_SEPARATOR).append(key).toString();
	}
	...
	/**
	 * Return message from exception messages properties file with given key and
	 * attributes.
	 * <p/>
	 *
	 * @param method The message string method key.
	...
	public String excMsg(String method, String key, String... attrs) {
		return MessageFormat.format(message(EXCEPTIONS_FILE, excProps, clazz, buildKey(method, key)), (Object[]) attrs);
	}
	...
	/**
	 * Log a message, with associated <code>Throwable</code> information from log
	 * messages catalog.
	 * <p>
	 * If the logger is currently enabled for the given message level then the given
	 * arguments are stored in a LogRecord which is forwarded to all registered
	 ...
	 * @param thrown <code>Throwable</code> associated with log message.
	 */
	public void log(final Level level, final String method, final String key, final Throwable thrown) {
	 ...
	 */
	public void log(final Level level, final String method, final String key, final Object param) {
	 ...
	 * @param params Array of parameters to the message.
	 */
	public void log(final Level level, final String method, final String key, final Object params[]) {
