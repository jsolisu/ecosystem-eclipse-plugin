...
public abstract class FetchLogPiped extends FetchLog implements Callable<TaskState> {
	...
	/**
	 * Constructs an instance of GlassFish server log fetcher depending on server
	 * being remote or local.
	 * <p/>
	 * Decision if server is local or remote depends on domains folder and domain
	 * name attributes stored in <code>PayaraServer</code> object.
	 * <p/>
	 *
	 ...
	 */
	public static FetchLogPiped create(final PayaraServer server, final boolean skip) {
	 ...
	 * <p/>
	 * Decision if server is local or remote depends on domains folder and domain
	 * name attributes stored in <code>PayaraServer</code> object. Log file is
	 * passed whole as is without skipping to the end.
	 * <p/>
	 *
	 * @param server GlassFish server for fetching server log.
	 * @return Newly created <code>FetchLog</code> instance.
	...
	/**
	 * Constructs an instance of GlassFish server log fetcher depending on server
	 * being remote or local with external {@link ExecutorService}.
	 * <p/>
	 * Decision if server is local or remote depends on domains folder and domain
	 * name attributes stored in <code>PayaraServer</code> object.
	 * <p/>
	 ...
	 * @return Newly created <code>FetchLog</code> instance.
	 */
	public static FetchLogPiped create(final ExecutorService executor, final PayaraServer server, final boolean skip) {
		boolean isLocal = NetUtils.isLocahost(server.getHost());
		FetchLogPiped fetchLog = isLocal ? new FetchLogLocal(executor, server, skip)
	 ...
	 * @return Newly created <code>FetchLog</code> instance.
	 */
	public static FetchLogPiped create(final ExecutorService executor, final PayaraServer server) {
	 ...
	 * <p/>
	 * Super class constructor will call <code>initInputStream</code> method which
	 * initializes <code>InputStream</code> as <code>PipedInputStream</code> before
	 * this constructor code is being executed. Here we can simply connect already
	 * initialized <code>PipedInputStream</code> with newly created
	...
	FetchLogPiped(final PayaraServer server, boolean skip) {
		...
		executor = new ThreadPoolExecutor(0, 1, 0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>(),
				(ThreadFactory) r -> {
					Thread t = new Thread(r,
							FetchLogPiped.class.getName() + server.getUrl() != null ? " (Localhost)" : server.getUrl());
					t.setDaemon(true);
					return t;
				});
	...
	FetchLogPiped(final ExecutorService executor, final PayaraServer server, boolean skip) {
		super(server, skip);
		final String METHOD = "init";
		this.eventListeners = new LinkedList();
		try {
	 ...
	 * constructor. It already exists when <code>FetchLogRemote</code> constructor
	 * is running so it may be used as argument for local
	 ...
	 * <p/>
	 *
	 * @param listener Listener for state change events in GlassFish log fetcher to
	 *                 be added. Value shall not be <code>null</code>.
	 * @throws FetchLogException When <code>listener</code> parameter is
	 *                           <code>null</code>.
	 */
	public final void addListener(final FetchLogEventListener listener) throws FetchLogException {
	...
	/**
	 * Remove all occurrences of log fetcher state change listener from listeners
	 * list.
	 * <p/>
	 *
	 * @param listener Listener for state change events in GlassFish log fetcher to
	 *                 be removed. Value shall not be <code>null</code>.
	 * @return Value of <code>true</code> when at least one listener was removed or
	 *         <code>false</code> otherwise.
	 * @throws FetchLogException When <code>listener</code> parameter is
	 *                           <code>null</code>.
	 */
	public final boolean removeListener(final FetchLogEventListener listener) throws FetchLogException {
	...
	/**
	 * Notify all GlassFish log fetcher state change listeners about state change
	 * event.
	 * <p/>
	 *
	...
	private TaskState stop() {
		...
		try {
			...
			throw new FetchLogException(LOGGER.excMsg(METHOD, "interrupted"), ie);
		} catch (ExecutionException ee) {
			throw new FetchLogException(LOGGER.excMsg(METHOD, "exception"), ee);
		} catch (CancellationException ce) {
			throw new FetchLogException(LOGGER.excMsg(METHOD, "cancelled"), ce);
		}
		return result;
	}

	/**
	 * Stop log lines reading task and close input and output streams used to access
	 * log lines received from server.
	 */
	@Override
	public void close() {
	 ...
	 * <p/>
	 *
	 * @return Returns <code>true</code> when task is still running or
	 *         <code>false></code> otherwise.
	 */
