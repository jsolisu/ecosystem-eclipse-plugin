 ...
 * and <code>version</code> and also verify if server is at least listening on
 * its administration port.
 * <p/>
...
public class ServerStatus implements Closeable {
	...
	public static class Result {
		 ...
		 */
		Result(final Status status, final IOException ioe, final TaskEvent failureEvent, final String serverName,
		 ...
		 * @param exceptionMeasage Exception message from command task.
		 */
		Result(final Status status, final Exception ex, final TaskEvent failureEvent, final String serverName,
				final String exceptionMeasage) {
			this.status = status;
			this.ioe = null;
			this.ex = ex;
		 ...
		 * @param exceptionMeasage Exception message from command task.
		 */
		Result(final Status status, final TaskEvent failureEvent, final String serverName,
				final String exceptionMeasage) {
			this.status = status;
			this.ioe = null;
			this.ex = null;
	 ...
	 * means that Locations command task execution finished successfully but it does
	 * not mean that administration command itself returned with
	 * <code>COMPLETED</code> status. When <code>SUCCESS</code> status is set,
	 * stored <code>result</code> value shall be examined too to see real
	 * administration command execution result.
	 */
	public static class ResultLocations extends Result {
		...
		/**
		 * Creates an instance of individual server status result for
		 ...
		 * @param exceptionMeasage Exception message from command task.
		 */
		ResultLocations(final ResultMap<String, String> result, final Status status, final TaskEvent failureEvent,
		 ...
		 * @param exceptionMeasage Exception message from command task.
		 */
		ResultLocations(final Status status, final Exception ex, final TaskEvent failureEvent, final String serverName,
				final String exceptionMeasage) {
		 ...
		 * @param exceptionMeasage Exception message from command task.
		 */
		ResultLocations(final Status status, final TaskEvent failureEvent, final String serverName,
				final String exceptionMeasage) {
			super(status, failureEvent, serverName, exceptionMeasage);
			this.result = null;
		}
	...
	public static class ResultVersion extends Result {
		 ...
		 * @param exceptionMeasage Exception message from command task.
		 */
		ResultVersion(final ResultString result, final Status status, final TaskEvent failureEvent,
				final String serverName, final String exceptionMeasage) {
			super(status, failureEvent, serverName, exceptionMeasage);
			this.result = result;
		}
		 ...
		 * @param exceptionMeasage Exception message from command task.
		 */
		ResultVersion(final Status status, final Exception ex, final TaskEvent failureEvent, final String serverName,
				final String exceptionMeasage) {
			super(status, ex, failureEvent, serverName, exceptionMeasage);
			this.result = null;
		}
		 ...
		 * @param exceptionMeasage Exception message from command task.
		 */
		ResultVersion(final Status status, final TaskEvent failureEvent, final String serverName,
				final String exceptionMeasage) {
			super(status, failureEvent, serverName, exceptionMeasage);
			this.result = null;
		}
	...
	private static abstract class Task implements TaskStateListener {
		...
		/**
		 * Format time value in miliseconds to be printed as value in seconds and
		 * miliseconds <code>s.ms<code>.
		 * <p/>
		 *
		...
		/**
		 * Task start time. Used for logging purposes. Value of <code>-1</code> means
		 * that start time was not set.
		 */
		long tmStart;
		...
		/**
		 * Compute task result waiting timeout based on task start time, actual time and
		 * minimal timeout limit.
		 * <p/>
		 *
		...
		long timeout(final boolean startup) {
			long timeout = (startup ? COMAND_STARTUP_TIMEOUT : COMAND_TIMEOUT) - System.currentTimeMillis() + tmStart;
		 ...
		 */
		@Override
		public void operationStateChanged(final TaskState newState, final TaskEvent event, final String[] args) {
	...
	/**
	 * Individual server status check task to verify if server administration port
	 * is alive.
	 * <p/>
	...
	private static class AdminPortTask extends Task {
		...
		private void closeSocket(final Socket socket) {
			try {
				...
				handleIOException(ioe, host, port, "Socket closing failed when connecting to {0}:{1}: {2}");
			}
		}
		 ...
		 */
		private void handleIOException(final IOException ioe, final String host, final int port, final String message) {
			final String METHOD = "handleIOException";
			String logMsg = MessageFormat.format(message,
					new Object[] { server.getName(), host, Integer.toString(port), ioe.getMessage() });
			if (tmStart >= 0 && LOGGER.isLoggable(Level.FINE)) {
				long tm = System.currentTimeMillis() - tmStart;
				LOGGER.log(Level.FINE, METHOD, "messageTm", new Object[] { logMsg, tm(tm) });
		 ...
		 * <p/>
		 *
		 * @return Returns <code>true</code> when server administration port is alive or
		...
		Result check() {
			...
			try {
				...
				handleIOException(ce, host, port, "[{0}] Port check could not connect to {1}:{2}: {3}");
				result = new Result(Status.FAILED, ce);
				return result;
			} catch (java.net.SocketTimeoutException ste) {
				handleIOException(ste, host, port, "[{0}] Port check timeout when connecting to {1}:{2}: {3}");
			...
			if (tmStart >= 0 && LOGGER.isLoggable(Level.FINE)) {
				long tm = System.currentTimeMillis() - tmStart;
				LOGGER.log(Level.FINE, METHOD, "success", new Object[] { tm(tm), server.getName() });
			}
	...
	private static class LocationsTask extends Task {
		...
		/**
		 * Startup mode. Triggers longer administration commands execution timeouts when
		 * <code>true</code>.
		 */
		 ...
		 *
		 * @param server  GlassFish server to be checked.
		 * @param startup Trigger startup mode. Triggers longer administration commands
		 *                execution timeouts when <code>true</code>.
		 */
		...
		private void logExceptionOnJoin(final Exception ex) {
			...
			LOGGER.log(Level.FINE, METHOD, "failed", new Object[] { tm(System.currentTimeMillis() - tmStart),
		...
		void join() {
			...
			try {
				taskResult = future.get(timeout(startup), TimeUnit.MILLISECONDS);
				result = new ResultLocations(taskResult, Status.SUCCESS, failureEvent, serverName, exceptionMeasage);
				LOGGER.log(Level.FINE, METHOD, "completed", tm(System.currentTimeMillis() - tmStart));
				// This means administration port is not responding.
			} catch (TimeoutException te) {
				result = new ResultLocations(Status.TIMEOUT, failureEvent, serverName, exceptionMeasage);
				logExceptionOnJoin(te);
				// Expected exceptions are handled in call() method so this
				// is something serious.
			} catch (ExecutionException ee) {
				result = new ResultLocations(Status.FATAL, ee, failureEvent, serverName, exceptionMeasage);
				logExceptionOnJoin(ee);
				// Interrupted after administration port check failed.
			} catch (InterruptedException ie) {
				result = new ResultLocations(Status.FAILED, ie, failureEvent, serverName, exceptionMeasage);
				logExceptionOnJoin(ie);
				// Cancelled after administration port check failed.
			} catch (CancellationException ce) {
				result = new ResultLocations(Status.FAILED, ce, failureEvent, serverName, exceptionMeasage);
				logExceptionOnJoin(ce);
			}
		}
	...
	/**
	 * Individual server status check task with <code>version</code> administration
	 * command execution.
	 */
	private static class VersionTask extends Task {
		...
		void join() {
			...
			try {
				taskResult = future.get(timeout(startup), TimeUnit.MILLISECONDS);
				result = new ResultVersion(taskResult, Status.SUCCESS, failureEvent, serverName, exceptionMeasage);
				LOGGER.log(Level.FINE, "completed", tm(System.currentTimeMillis() - tmStart));
				// This means administration port is not responding.
			} catch (TimeoutException te) {
				result = new ResultVersion(Status.TIMEOUT, failureEvent, serverName, exceptionMeasage);
				logExceptionOnJoin(te);
				// Expected exceptions are handled in call() method so this
				// is something serious.
			} catch (ExecutionException ee) {
				result = new ResultVersion(Status.FATAL, ee, failureEvent, serverName, exceptionMeasage);
				logExceptionOnJoin(ee);
				// Interrupted after administration port check failed.
			} catch (InterruptedException ie) {
				result = new ResultVersion(Status.FAILED, ie, failureEvent, serverName, exceptionMeasage);
				logExceptionOnJoin(ie);
				// Cancelled after administration port check failed.
			} catch (CancellationException ce) {
				result = new ResultVersion(Status.FAILED, ce, failureEvent, serverName, exceptionMeasage);
				logExceptionOnJoin(ce);
			}
		}
	...
	/**
	 * Asynchronous server status checks start time. Used for logging purposes.
	 * Value of <code>-1</code> means that start time was not set.
	 */
	 ...
	 * <p/>
	 * Method {@link #close()} must be called at the end to release system
	 * resources.
	 * <p/>
	 *
	 * @param server  GlassFish server to be checked.
	...
	public Version getVersion() {
		String versionStr = versionTask.result != null && versionTask.result.result != null
	 ...
	 * administration port has not been available, remaining command tasks are
	 * canceled.
	 */
	public void check() {
