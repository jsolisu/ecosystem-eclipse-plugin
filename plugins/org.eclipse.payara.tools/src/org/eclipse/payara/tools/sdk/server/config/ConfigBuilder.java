 ...
 * <p/>
 * Instance of library builder for single version of GlassFish server. Version
 * of GlassFish server is supplied with first configuration getter call. Each
 * subsequent configuration getter call on the same instance must be used with
 * the same GlassFish version.
 * <p/>
 * XML configuration file is read just once with first configuration getter
 * call. Returned values are cached for subsequent getter calls which are very
 * fast.
 * <p/>
 *
...
public class ConfigBuilder {
	...
	/**
	 * Build <code>List</code> of <code>GlassFishLibrary</code> objects representing
	 * libraries found in particular GlassFish server installation.
	 * <p/>
	 *
	 ...
	 *         installation.
	 */
	private static List<GlassFishLibrary> getLibraries(final List<LibraryNode> libConfigs, final File classpathHome,
		...
		try {
			for (LibraryNode libConfig : libConfigs) {
				...
				result.add(new GlassFishLibrary(libConfig.libraryID, buildUrls(classpath),
						buildUrls(javadocs, javadocUrls), libConfig.javadocs.getLookups(), buildUrls(sources),
	 ...
	 * @return List of <code>URL</code> objects containing content of both supplied
	 *         lists.
	 */
	private static List<URL> buildUrls(final List<File> files, final List<URL> urls) {
	 ...
	 * <p/>
	 *
	 * @param config        Library builder configuration. Should not be
	 *                      <code>null</code>.
	 ...
	 */
	ConfigBuilder(final String classpathHome, final String javadocsHome, final String srcHome) {
	 ...
	 * @param srcHome       Source code search prefix.
	 */
	ConfigBuilder(final File classpathHome, File javadocsHome, File srcHome) {
	...
	/**
	 * Internal version check to avoid usage of a single builder instance for
	 * multiple GlassFish versions.
	 * <p/>
	 *
	 * @param version GlassFish version being checked.
	 * @throws ServerConfigException when builder is used with multiple GlassFish
	 *                               versions.
	 */
	private void versionCheck(final Version version) throws ServerConfigException {
		if (this.version == null) {
			this.version = version;
		} else if (this.version != version) {
			throw new ServerConfigException("Library builder was already used for GlassFish " + this.version
					+ " use new instance for GlassFish" + version);
		}
	}
	...
	private void fetch(final Version version) {
		synchronized (this) {
			if (!fetchDone) {
				GlassFishConfig configAdapter = GlassFishConfigManager
						.getConfig(ConfigBuilderProvider.getBuilderConfig(version));
				List<LibraryNode> libConfigs = configAdapter.getLibrary();
				libraryCache = getLibraries(libConfigs, classpathHome, javadocsHome, srcHome);
				javaEEConfigCache = new GlassFishJavaEEConfig(configAdapter.getJavaEE(), classpathHome);
				javaSEConfigCache = new GlassFishJavaSEConfig(configAdapter.getJavaSE());
	 ...
	 * <p/>
	 * This method shall not be used with multiple GlassFish versions for the same
	 * instance of {@link ConfigBuilder} class.
	 * <p/>
	 *
	 ...
	 *                               GlassFish versions.
	 */
	public List<GlassFishLibrary> getLibraries(final Version version) throws ServerConfigException {
	 ...
	 *
	 * @param version GlassFish version.
	 * @return GlassFish JavaEE configuration for provided GlassFish of given
	 *         version.
	 * @throws ServerConfigException when builder instance is used with multiple
	 *                               GlassFish versions.
	 */
	public GlassFishJavaEEConfig getJavaEEConfig(final Version version) throws ServerConfigException {
		versionCheck(version);
		if (!fetchDone) {
			fetch(version);
		}
	 ...
	 *
	 * @param version GlassFish version.
	 * @return GlassFish JavaSE configuration for provided GlassFish of given
	 *         version.
	 * @throws ServerConfigException when builder instance is used with multiple
	 *                               GlassFish versions.
	 */
	public GlassFishJavaSEConfig getJavaSEConfig(final Version version) throws ServerConfigException {
		versionCheck(version);
		if (!fetchDone) {
			fetch(version);
		}
