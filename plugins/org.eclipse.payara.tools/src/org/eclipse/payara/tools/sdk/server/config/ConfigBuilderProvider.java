 ...
 * instances. Because {@link ConfigBuilder} class instance shall not be used for
 * multiple GlassFish server versions there must be one configuration class
 * instance for every single GlassFish server version. Also every single server
 * instance has it's own directory structure which is used to search for
 * modules. Because of that every single GlassFish server instance must have
 * it's own configuration builder. Configuration builder is created with first
 * request for given server version and reused for every subsequent request.
 * <p/>
 *
 * @author Tomas Kraus
 */
public class ConfigBuilderProvider {
	 ...
	 * instance.
	 * <p/>
	 * Allows to free resources when configuration builder instance will no more be
	 * needed (e.g. GlassFish server entity is being destroyed).
	 * <p/>
	 *
