...
import org.eclipse.payara.tools.sdk.server.parser.JavaEEModuleReader;
import org.eclipse.payara.tools.sdk.server.parser.JavaEEProfileCheckReader;
import org.eclipse.payara.tools.sdk.server.parser.JavaEEProfileReader;
...
public class JavaEESet extends JavaSet {
	 ...
	 */
	public JavaEESet(final List<JavaEEModuleReader.Module> modules, final List<JavaEEProfileReader.Profile> profiles,
			final List<JavaEEProfileCheckReader.Check> checks, final String version) {
		super(version);
	...
	/**
	 * Get existing cached check result or run a new check if no such cached check
	 * result exists.
	 * <p/>
	 * Cached check results depends on provided <code>classpathHome</code> and they
	 * should be reset before <code>classpathHome</code> will be changed.
	 * <p/>
	 *
	...
	/**
	 * Build {@link Set} of {@link ModuleType} for known module types retrieved from
	 * XML elements.
	 * <p/>
	 *
	 * @param classpathHome Classpath search prefix.
	...
	public Set<JavaEEProfile> profiles(final File classpathHome) {
		...
		if (size > 0) {
			for (JavaEEProfileReader.Profile profileFromXML : profiles) {
				...
				if (profile != null && check(profileFromXML.getCheck(), classpathHome)) {
