...
import org.eclipse.payara.tools.sdk.server.config.FileSet;
...
public class ConfigReaderServer extends TreeParser.NodeListener implements XMLReader {
	 ...
	 * @throws ServerConfigException when more than one <code>javaee</code> XML
	 *                               elements were found.
	 */
	@Override
	public void endNode(final String qname) throws SAXException {
		if ("library".equals(qname)) {
			FileSet classpath = new FileSet(classpathReader.getPaths(), classpathReader.getFilesets());
			FileSet javadocs = new FileSet(javadocsReader.getPaths(), javadocsReader.getLinks(),
					javadocsReader.getFilesets(), javadocsReader.getLookups());
			FileSet sources = new FileSet(sourcesReader.getPaths(), sourcesReader.getFilesets());
			LibraryNode config = new LibraryNode(actualLibID, classpath, javadocs, sources);
