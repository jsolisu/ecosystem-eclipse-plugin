...
public class ConfigReaderTools extends AbstractReader implements XMLReader {
	...
	public List<TreeParser.Path> getPathsToListen() {
		...
		paths.add(new TreeParser.Path(toolsAsadminReader.getPath(), toolsAsadminReader));
		return paths;
	}
	 ...
	 * @throws ServerConfigException when more than one <code>tools</code> XML
	 *                               elements were found.
	 */
	@Override
	public void endNode(final String qname) throws SAXException {
		if (NODE.equals(qname)) {
			if (tools != null) {
				throw new SAXException("Multiple " + NODE + " XML element is not allowed.");
			}
