...
public class JvmConfigReader extends NodeListener implements XMLReader {
	...
	/**
	 * Holds all key-value pairs representing attributes of jvm-config tag. These
	 * are used for computing the classpath.
	 */
			...
			public void readAttributes(String qname, Attributes attributes) throws SAXException {
				...
				if (serverConfigName == null || serverConfigName.length() == 0) {
					if (serverName.equals(attributes.getValue("name"))) { // NOI18N
						...
						// Logger.getLogger("glassfish").finer("DOMAIN.XML: Server profile defined by "
						// + serverConfigName);
						// // NOI18N
					}
				}
		...
		return new TreeParser.NodeListener() {
			...
			public void readAttributes(String qname, Attributes attributes) throws SAXException {
				...
				if (serverConfigName != null && serverConfigName.equals(attributes.getValue("name"))) { // NOI18N
					readConfig = true;
					// Logger.getLogger("glassfish").finer("DOMAIN.XML: Reading JVM options from
					// server profile " +
	...
	public static class JvmOption {
		...
		public String toString() {
			...
			return String.format("[%s|%s]%s", minVersion.isPresent() ? minVersion.get() : "",
