...
/**
 * Reads configuration of network listeners. For each listener returns one
 * {@link HttpData} object that contains port number, protocol and information
 * whether this protocol is secured.
 * <p/>
 *
...
public class NetworkListenerReader extends TargetConfigReader implements XMLReader {
	...
	public void readAttributes(String qname, Attributes attributes) throws SAXException {
		...
		/*
		 * <network-listeners> <thread-pool max-thread-pool-size="20"
		 * min-thread-pool-size="2" thread-pool-id="http-thread-pool"
		 * max-queue-size="4096"></thread-pool> <network-listener port="8080"
		 ...
		 * port="8181" enabled="false" protocol="http-listener-2" transport="tcp"
		 ...
		 * name="admin-listener" thread-pool="http-thread-pool"></network-listener>
		 * </network-listeners>
		 */
		if (readData) {
			try {
				...
				if (id != null && id.length() > 0) {
					...
					LOGGER.log(Level.INFO, METHOD, "port", new Object[] { Integer.toString(port),
							Boolean.toString(enabled), Boolean.toString(secure) });
					if (enabled) {
