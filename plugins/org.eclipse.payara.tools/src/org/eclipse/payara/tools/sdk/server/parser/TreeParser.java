...
public final class TreeParser extends DefaultHandler {
	...
	public void startElement(String uri, String localname, String qname, Attributes attributes) throws SAXException {
		...
		if (skipping != null) {
			...
			if (isFinestLoggable) {
				LOGGER.log(Level.FINEST, METHOD, "skipDescend", new Object[] { depth, qname });
			}
		} else {
			...
			if (child != null) {
				...
				if (isFinestLoggable) {
					LOGGER.log(Level.FINEST, METHOD, "skipStart", new Object[] { depth, qname });
				}
			}
		}
	}
		...
		if (skipping != null) {
			...
			if (--depth == 0) {
				if (!skipping.equals(qname)) {
					LOGGER.log(Level.WARNING, METHOD, "doesNotMatch", new Object[] { skipping, qname, depth });
				}
				skipping = null;
	...
	private static TreeParser.Node buildTree(List<TreeParser.Path> paths) {
		...
		for (TreeParser.Path path : paths) {
			...
			if (root == null) {
				if (isFinerLoggable) {
					LOGGER.log(Level.FINER, METHOD, "createdRootNode", parts[0]);
				}
			...
			for (int i = 1; i < parts.length; i++) {
				if (parts[i] != null && parts[i].length() > 0) {
					...
					if (existing != null) {
						if (isFinerLoggable) {
							LOGGER.log(Level.FINER, METHOD, "existing", new Object[] { parts[i], Integer.toString(i) });
						}
						rover = existing;
					} else {
						if (isFinerLoggable) {
							LOGGER.log(Level.FINER, METHOD, "add", new Object[] { parts[i], Integer.toString(i) });
						}
						rover = rover.addChild(parts[i]);
					}
				} else {
					LOGGER.log(Level.WARNING, METHOD, "broken", new Object[] { path, Integer.toString(i) });
				}
			}
			if (rover != null) {
