 ...
 *
 * <p>
 * A few methods from ServerDelegate are overridden here, while a whole slew of
 * central server like functionality is put here as well.
 * </p>
 *
 * <p>
...
public final class PayaraServer extends ServerDelegate implements IURLProvider {
	...
	public ServerPort[] getServerPorts() {
		try {
			return new ServerPort[] { new ServerPort("adminserver", "Admin Server Port", getAdminPort(), "HTTP"),
	...
	public static IPath getDefaultDomainDir(IPath serverLocation) {
		...
		return serverLocation.append(DEFAULT_SERVER_DIR_NAME).append(DEFAULT_DOMAIN_DIR_NAME)
	...
	private IModule[] doGetParentModules(IModule module) {
		...
		for (IModule earModule : getModules("jst.ear")) {
			IEnterpriseApplication ear = (IEnterpriseApplication) earModule.loadAdapter(IEnterpriseApplication.class,
					null);
