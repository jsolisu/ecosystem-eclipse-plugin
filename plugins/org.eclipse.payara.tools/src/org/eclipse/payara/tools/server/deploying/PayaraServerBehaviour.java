 ...
 * on the Payara / GlassFish server such as deploy/undeploy etc.
 * Starting/restarting is delegated to {@link PayaraServerLaunchDelegate}.
 *
 * <p>
...
public final class PayaraServerBehaviour extends ServerBehaviourDelegate implements ServerStateListener {
	...
	private void publishDeployedDirectory(int kind, int deltaKind, Properties publishProperties, IModule module[],
		...
		if (deltaKind == REMOVED) {
			...
			if (needARedeploy) {
				...
				try {
					ServerAdmin.executeOn(getPayaraServerDelegate()).command(command).onNotCompleted(result -> {
				...
				try {
					ServerAdmin
							.executeOn(getPayaraServerDelegate()).command(new CommandDeploy(name, null, archivePath,
									contextRoot, getDeploymentProperties(), new File[0]))
	...
	private void undeploy(String moduleName) throws CoreException {
		try {
			ServerAdmin.executeOn(getPayaraServerDelegate()).command(new CommandUndeploy(moduleName)).timeout(520)
	...
	public void updateHttpPort() throws HttpPortUpdateException {
		...
		Future<ResultMap<String, String>> future = exec(getPayaraServerDelegate(),
